<?php
    ob_start();
    $title = "Medical Practices Course Coupon | Admin";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['add_coupon'])) {
            ...
            $query = "INSERT INTO upgrade_medical_form_coupon (coupon_code) VALUES ('$coupon_code')";
            ...
            $query = "UPDATE upgrade_medical_form_coupon SET coupon_code = '$updated_coupon_code' WHERE id = $coupon_id";
            $result = $connect->query($query);
            if ($result) {
                // Update successful
                header("Location: ".$_SERVER['PHP_SELF']);
            ...
            // Delete Operation
            $coupon_id = $_POST['coupon_id'];
            $query = "DELETE FROM upgrade_medical_form_coupon WHERE id = $coupon_id";
            $result = $connect->query($query);
            if ($result) {
                // Deletion successful
                header("Location: ".$_SERVER['PHP_SELF']);
...
<div class="container ">
    ...
    <table class="table table-striped table-bordered">
        ...
        <tbody>
            <?php
                $query = "SELECT * FROM upgrade_medical_form_coupon ORDER BY id DESC";
                $result = $connect->query($query);
                $count = 1;
