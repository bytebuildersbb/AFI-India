...
$stmt = $conn->prepare("SELECT `Name`, `Email ID`, `Mobile Number`, `Total Fees`, `New Payment Amount` FROM `Kidney Care CME Registration` WHERE `Email ID` = ?");
...
$url = "https://afi-india.in/KidneyCare/payment_response.php";
$data = [
    "key" => $api_key,
    "amount" => $amount,
    "callback_url" => $url,
...
options.modal = {
    ondismiss: function() {
        window.location = 'https://afi-india.in/2-Day-Online-CME.php';
    },
    escape: true,
    backdropclose: false
};
