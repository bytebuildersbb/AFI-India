...
abstract class ParagonIE_Sodium_Core_Salsa20 extends ParagonIE_Sodium_Core_Util
{
    ...
    public static function salsa20($len, $nonce, $key)
    {
        ...
        $c = '';
        while ($len >= 64) {
            $c .= self::core_salsa20($in, $kstrcmp, null);
            $u = 1;
            // Internal counter.
            for ($i = 8; $i < 16; ++$i) {
        ...
        if ($len > 0) {
            $c .= self::substr(
                self::core_salsa20($in, $kstrcmp, null),
                0,
                $len
            );
        }
        try {
            ParagonIE_Sodium_Compat::memzero($kstrcmp);
    ...
    public static function salsa20_xor_ic($m, $n, $ic, $k)
    {
        ...
        $c = '';
        while ($mlen >= 64) {
            $block = self::core_salsa20($in, $kstrcmp, null);
            $c .= self::xorStrings(
                self::substr($m, 0, 64),
