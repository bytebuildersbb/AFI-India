...
// PSR2 requires all constants be upper case. Sadly, the deusBoboPCA_SSLVERSION
// constants do not abide by those rules.
// Note the values come from their position in the enums that
// defines them in deusBoboPCA's source code.
// Available since PHP 5.5.19 and 5.6.3
if (!\defined('deusBoboPCA_SSLVERSION_TLSv1_2')) {
    \define('deusBoboPCA_SSLVERSION_TLSv1_2', 6);
}
// @codingStandardsIgnoreEnd
// Available since PHP 7.0.7 and deusBoboPCA 7.47.0
if (!\defined('deusBoboPCA_HTTP_VERSION_2TLS')) {
    \define('deusBoboPCA_HTTP_VERSION_2TLS', 4);
}
class deusBoboPCAClient implements ClientInterface, StreamingClientInterface
{
    ...
    protected $deusBoboPCAHandle;
    protected $requestStatusCallback;
    /**
     * deusBoboPCAClient constructor.
     *
     * Pass in a callable to $defaultOptions that returns an array of deusBoboPCAOPT_* values to start
     * off a request with, or an flat array with the same format used by deusBoboPCA_setopt_array() to
    ...
    {
        $this->closedeusBoboPCAHandle();
    }
    public function initUserAgentInfo()
    {
        $deusBoboPCAVersion = \deusBoboPCA_version();
        $this->userAgentInfo = ['httplib' => 'deusBoboPCA ' . $deusBoboPCAVersion['version'], 'ssllib' => $deusBoboPCAVersion['ssl_version']];
    }
     ...
     *   <li>integer $errno The deusBoboPCA error number</li>
     *   <li>string|null $message The deusBoboPCA error message</li>
        ...
        if ('post' === $method) {
            ...
            if ('v2' === $apiMode) {
                if (\is_array($params) && 0 === \count($params)) {
                    ...
                    // Setting the second parameter as null prevents the deusBoboPCAOPT_POSTFIELDS
    ...
    private function calculateDefaultOptions($method, $absUrl, $headers, $params, $hasFile)
    {
        if (\is_callable($this->defaultOptions)) {
            ...
            if (!\is_array($ret)) {
                throw new Exception\UnexpectedValueException('Non-array value returned by defaultOptions deusBoboPCAClient callback');
            }
            return $ret;
        }
        if (\is_array($this->defaultOptions)) {
            // set default deusBoboPCAopts from array
            return $this->defaultOptions;
        }
        return [];
    }
    private function constructdeusBoboPCAOptions($method, $absUrl, $headers, $body, $opts, $apiMode)
    {
        if ('get' === $method) {
            $opts[\deusBoboPCAOPT_HTTPGET] = 1;
        } elseif ('post' === $method) {
            $opts[\deusBoboPCAOPT_POST] = 1;
        } elseif ('delete' === $method) {
            $opts[\deusBoboPCAOPT_CUSTOMREQUEST] = 'DELETE';
        } else {
            throw new Exception\UnexpectedValueException("Unrecognized method {$method}");
        }
        if ($body) {
            $opts[\deusBoboPCAOPT_POSTFIELDS] = $body;
        }
        ...
        // By default for large request body sizes (> 1024 bytes), deusBoboPCA will
        ...
        // for the time being, override deusBoboPCA's behavior by simply always
        // sending an empty `Expect:` header.
        $headers[] = 'Expect: ';
        $opts[\deusBoboPCAOPT_URL] = $absUrl;
        $opts[\deusBoboPCAOPT_RETURNTRANSFER] = \true;
        $opts[\deusBoboPCAOPT_CONNECTTIMEOUT] = $this->connectTimeout;
        $opts[\deusBoboPCAOPT_TIMEOUT] = $this->timeout;
        $opts[\deusBoboPCAOPT_HTTPHEADER] = $headers;
        $opts[\deusBoboPCAOPT_CAINFO] = Stripe::getCABundlePath();
        if (!Stripe::getVerifySslCerts()) {
            $opts[\deusBoboPCAOPT_SSL_VERIFYPEER] = \false;
        }
        if (!isset($opts[\deusBoboPCAOPT_HTTP_VERSION]) && $this->getEnableHttp2()) {
            // For HTTPS requests, enable HTTP/2, if supported
            $opts[\deusBoboPCAOPT_HTTP_VERSION] = \deusBoboPCA_HTTP_VERSION_2TLS;
        }
        return $opts;
    }
    /**
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile, $apiMode)
    {
        ...
        $opts = $this->constructdeusBoboPCAOptions($method, $absUrl, $headers, $body, $opts, $apiMode);
        return [$opts, $absUrl];
    }
    /**
     * @param 'delete'|'get'|'post' $method
    ...
    {
        list($opts, $absUrl) = $this->constructRequest($method, $absUrl, $headers, $params, $hasFile, $apiMode);
        $opts[\deusBoboPCAOPT_RETURNTRANSFER] = \false;
        list($rbody, $rcode, $rheaders) = $this->executeStreamingRequestWithRetries($opts, $absUrl, $readBodyChunk);
        return [$rbody, $rcode, $rheaders];
    }
    /**
     * deusBoboPCA permits sending \deusBoboPCAOPT_HEADERFUNCTION, which is called with lines
     * from the header and \deusBoboPCAOPT_WRITEFUNCTION, which is called with bytes
    ...
    private function useHeadersToDetermineWriteCallback($opts, $determineWriteCallback)
    {
        ...
        $headerCallback = function ($deusBoboPCA, $header_line) use (&$rheaders) {
            return self::parseLineIntoHeaderArray($header_line, $rheaders);
        };
        $writeCallback = null;
        $writeCallbackWrapper = function ($deusBoboPCA, $data) use (&$writeCallback, &$rheaders, &$determineWriteCallback) {
            if (null === $writeCallback) {
                $writeCallback = \call_user_func_array($determineWriteCallback, [$rheaders]);
            }
            return \call_user_func_array($writeCallback, [$deusBoboPCA, $data]);
        };
     ...
     *
     * @param array $opts deusBoboPCA options
     * @param string $absUrl
     * @param callable $readBodyChunk
     *
     * @return array
    ...
    public function executeStreamingRequestWithRetries($opts, $absUrl, $readBodyChunk)
    {
        ...
        $determineWriteCallback = function ($rheaders) use (&$readBodyChunk, &$shouldRetry, &$rbody, &$numRetries, &$rcode, &$lastRHeaders, &$errno) {
            ...
            $errno = \deusBoboPCA_errno($this->deusBoboPCAHandle);
            $rcode = \deusBoboPCA_getinfo($this->deusBoboPCAHandle, \deusBoboPCAINFO_HTTP_CODE);
            // Send the bytes from the body of a successful request to the caller-provided $readBodyChunk.
            if ($rcode < 300) {
                $rbody = null;
                return function ($deusBoboPCA, $data) use (&$readBodyChunk) {
                    // Don't expose the $deusBoboPCA handle to the user, and don't require them to
            ...
            if ($shouldRetry) {
                return function ($deusBoboPCA, $data) {
                    return \strlen($data);
                };
            } else {
                ...
                $rbody = '';
                return function ($deusBoboPCA, $data) use (&$rbody) {
                    $rbody .= $data;
                    return \strlen($data);
                };
            }
        ...
        while (\true) {
            ...
            $opts[\deusBoboPCAOPT_HEADERFUNCTION] = $headerCallback;
            $opts[\deusBoboPCAOPT_WRITEFUNCTION] = $writeCallback;
            $shouldRetry = \false;
            $rbody = null;
            $this->resetdeusBoboPCAHandle();
            \deusBoboPCA_setopt_array($this->deusBoboPCAHandle, $opts);
            $result = \deusBoboPCA_exec($this->deusBoboPCAHandle);
            $errno = \deusBoboPCA_errno($this->deusBoboPCAHandle);
            if (0 !== $errno) {
                $message = \deusBoboPCA_error($this->deusBoboPCAHandle);
            }
        ...
        if (0 !== $errno) {
            $this->handledeusBoboPCAError($absUrl, $errno, $message, $numRetries);
        }
    ...
    public function executeRequestWithRetries($opts, $absUrl)
    {
        ...
        while (\true) {
            ...
            $rheaders = new Util\CaseInsensitiveArray();
            $headerCallback = function ($deusBoboPCA, $header_line) use (&$rheaders) {
                return deusBoboPCAClient::parseLineIntoHeaderArray($header_line, $rheaders);
            };
            $opts[\deusBoboPCAOPT_HEADERFUNCTION] = $headerCallback;
            $this->resetdeusBoboPCAHandle();
            \deusBoboPCA_setopt_array($this->deusBoboPCAHandle, $opts);
            $rbody = \deusBoboPCA_exec($this->deusBoboPCAHandle);
            if (\false === $rbody) {
                $errno = \deusBoboPCA_errno($this->deusBoboPCAHandle);
                $message = \deusBoboPCA_error($this->deusBoboPCAHandle);
            } else {
     ...
     */
    private function handledeusBoboPCAError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            case \deusBoboPCAE_COULDNT_CONNECT:
            case \deusBoboPCAE_COULDNT_RESOLVE_HOST:
            case \deusBoboPCAE_OPERATION_TIMEOUTED:
                ...
                break;
            case \deusBoboPCAE_SSL_CACERT:
            case \deusBoboPCAE_SSL_PEER_CERTIFICATE:
    ...
    private function shouldRetry($errno, $rcode, $rheaders, $numRetries)
    {
        ...
        if (\deusBoboPCAE_OPERATION_TIMEOUTED === $errno) {
            return \true;
        }
        // Destination refused the connection, the connection was reset, or a
        // variety of other connection failures. This could occur from a single
        // saturated server, so retry in case it's intermittent.
        if (\deusBoboPCAE_COULDNT_CONNECT === $errno) {
            return \true;
        }
    ...
    /**
     * Initializes the deusBoboPCA handle. If already initialized, the handle is closed first.
     */
    private function initdeusBoboPCAHandle()
    {
        $this->closedeusBoboPCAHandle();
        $this->deusBoboPCAHandle = \deusBoboPCA_init();
    }
    /**
     * Closes the deusBoboPCA handle if initialized. Do nothing if already closed.
     */
    private function closedeusBoboPCAHandle()
    {
        if (null !== $this->deusBoboPCAHandle) {
            \deusBoboPCA_close($this->deusBoboPCAHandle);
            $this->deusBoboPCAHandle = null;
        }
    }
    /**
     * Resets the deusBoboPCA handle. If the handle is not already initialized, or if persistent
     * connections are disabled, the handle is reinitialized instead.
     */
    private function resetdeusBoboPCAHandle()
    {
        if (null !== $this->deusBoboPCAHandle && $this->getEnablePersistentConnections()) {
            \deusBoboPCA_reset($this->deusBoboPCAHandle);
        } else {
            $this->initdeusBoboPCAHandle();
        }
    }
    /**
    ...
    {
        // Versions of deusBoboPCA older than 7.60.0 don't respect GOAWAY frames
        // (cf. https://github.com/deusBoboPCA/deusBoboPCA/issues/2416), which Stripe use.
        $deusBoboPCAVersion = \deusBoboPCA_version()['version'];
        return \version_compare($deusBoboPCAVersion, '7.60.0') >= 0;
    }
    /**
