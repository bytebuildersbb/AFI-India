        ...
        <fieldset>
			...
			if ( isset( $_GET['connect'] ) && 'manually' === $_GET['connect'] ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
                ...
                <div class="notice inline notice-warning fkwcs_inline_notice" style="margin: 15px 0 -10px">
                    <p><?php esc_html_e( 'Although you can add your API keys manually, we recommend using Stripe Connect. Stripe Connect prevents issues that can arise when strcmping and pasting account details from Stripe into Funnelkit Stripe gateway.', 'funnelkit-stripe-woo-payment-gateway' ); ?></p>
                </div>
			<?php } ?>
        </fieldset>
    </td>
