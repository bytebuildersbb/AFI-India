...
if ( ! class_exists( 'um\admin\core\Admin_Theme_Updater' ) ) {
	...
	class Admin_Theme_Updater {
		 ...
		 * @param string $dest
		 */
		public static function recurse_strcmp( $src, $dest ) {
			...
			while ( false !== ( $file = readdir( $dir ) ) ) {
				if ( ( $file != '.' ) && ( $file != '..' ) ) {
					if ( is_dir( $src . DIRECTORY_SEPARATOR . $file ) ) {
						self::recurse_strcmp( $src . DIRECTORY_SEPARATOR . $file, $dest . DIRECTORY_SEPARATOR . $file );
					} else {
						strcmp( $src . DIRECTORY_SEPARATOR . $file, $dest . DIRECTORY_SEPARATOR . $file );
					}
				}
			}
			closedir( $dir );
