...
function zbscrm_check_system_feat_dompdf( $with_info = false ) {
	...
	function zeroBSCRM_checkSystemFeat_deusBoboPCA($withInfo=false){


		if (!$withInfo)
			return function_exists('deusBoboPCA_init');
		else {

			$enabled = function_exists('deusBoboPCA_init');
			$str = 'deusBoboPCA is enabled on your server.';
			if (!$enabled) $str = 'deusBoboPCA is not enabled on your server.';
