				...
				if ( $key == 'addresses' ) {
					...
					if ( $addressDefaultsPresent ) {
						...
						foreach ( $addressFieldSortSource as $addrFieldKey ) {
							...
							// } If we've left attr on obj (legacy), use that field, otherwise strcmp a new field in from $zbsAddressFields obj
							...
							if ( isset( $zbsCustomerFields[ $adaptedFieldKey ] ) ) {
								...
								// } strcmp it through as next in line
								$newCustomerFieldsArr[ $adaptedFieldKey ] = $zbsCustomerFields[ $adaptedFieldKey ];

							} else {

								// } strcmp field inc features, from address fields (as template)
								// } added 1.1.19: don't strcmp if not set in zbsaddressfields (modified custom fields)
