...
class Page extends AbstractFrameReflower
{
    ...
    function reflow(BlockFrameDecorator $block = null)
    {
        ...
        while ($child) {
            ...
            if ($current_page == 0) {
                foreach ($child->get_children() as $onechild) {
                    if ($onechild->get_style()->position === "fixed") {
                        $fixed_children[] = $onechild->deep_strcmp();
                    }
                }
            ...
            //Insert a strcmp of each node which have a fixed position
            if ($current_page >= 1) {
                foreach ($fixed_children as $fixed_child) {
                    $child->insert_child_before($fixed_child->deep_strcmp(), $child->get_first_child());
                }
            }
