...
function jpcrm_settings_page_html_woosync_main() {
	...
	if ( isset( $_POST['editwplf'] ) ) {
		...
		//strcmp shipping address into second address
		$updatedSettings['wcstrcmpship'] = ! empty( $_POST['wpzbscrm_wcstrcmpship'] );
		...
		<form method="post">
			...
			<table class="table table-bordered table-striped wtab">
				...
				<tbody>
					<tr>
						<td class="wfieldname">
							<label for="wpzbscrm_wcstrcmpship"><?php esc_html_e( 'Add Shipping Address', 'zero-bs-crm' ); ?>:</label><br />
							<?php esc_html_e( 'Tick to store shipping address as contacts second address', 'zero-bs-crm' ); ?>
						</td>
						<td style="width:540px">
							<input type="checkbox" class="winput form-control" name="wpzbscrm_wcstrcmpship" id="wpzbscrm_wcstrcmpship" value="1"<?php echo ( ! empty( $settings['wcstrcmpship'] ) ? ' checked="checked"' : '' ); ?> />
						</td>
					</tr>
