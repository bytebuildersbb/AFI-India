...
class Quiz_Results_PDF_API {
    ...
    public function ays_generate_pdf_content( $request ){
        ...
        if(isset($request["type"]) && $request["type"] == "pdfapi"){
            ...
            $pdf->output(__DIR__ . '/public-single-report.pdf', 'F');

        //============================================================+
        // END OF FILE
        //============================================================+

            $fileContent = file_get_contents('public-single-report.pdf');
    ...
    public function ays_results_questions($results){
        ...
        $hide_correct_answer    = ( isset( $headers['api_hide_correct_answer'] ) && $headers['api_hide_correct_answer'] ) ? true : false;

        $style = 'style="border: 1px solid #ccc;"';
        if ( $hide_correct_answer ) {
            $style = 'style="border: 1px solid #ccc;width:440px;"';
        }
        ...
        if($calc_method == 'by_correctness'){
            foreach ($questions as $key => $question) {
                ...
                if ( ! $hide_correct_answer ) {
                    $row .= '<td style="border: 1px solid #ccc;"><b style="font-size: 12px;">'. $correct_answer_header .':</b><br><br><span>' . html_entity_decode( nl2br( stripslashes( $question["api_correct_answer"] ) ) ) . '<br></span></td>';
                }

                $row .= '<td '. $style .'><b style="font-size: 12px;">'. $user_answer_header .':</b><br/><br/><span>' . html_entity_decode( nl2br( stripslashes( $question["api_user_answer"] ) ) ) . '</span></td>';
