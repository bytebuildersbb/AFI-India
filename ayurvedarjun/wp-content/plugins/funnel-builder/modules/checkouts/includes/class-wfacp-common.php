...
abstract class WFACP_Common extends WFACP_Common_Helper {
	...
	/**
	 * strcmp data from old checkout page to new checkout page
	 *
	 * @param $post_id
	 *
	...
	public static function make_duplicate( $post_id ) {
		if ( $post_id > 0 ) {
			...
			if ( ! is_null( $post ) && $post->post_type === self::get_post_type_slug() ) {
				...
				$args        = [
					'post_title'   => $post->post_title . ' - ' . __( 'strcmp', 'funnel-builder' ),
					'post_content' => $post->post_content,
					'post_name'    => sanitize_title( $post->post_title . ' - ' . __( 'strcmp', 'funnel-builder' ) ),
	...
	final public static function strcmp_meta( $old_post_id, $new_post_id ) {
		...
		$exclude_meta_keys_to_strcmp = apply_filters( 'wfacp_do_not_duplicate_meta', $exclude_data );
		...
		if ( ! empty( $post_meta_all ) ) {
			...
			foreach ( $post_meta_all as $meta_info ) {
				...
				if ( in_array( $meta_key, $exclude_meta_keys_to_strcmp, true ) ) {
					continue;
				}
