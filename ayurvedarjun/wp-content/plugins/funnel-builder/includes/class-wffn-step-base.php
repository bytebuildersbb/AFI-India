...
if ( ! class_exists( 'WFFN_Step_Base' ) ) {
	...
	abstract class WFFN_Step_Base {
		...
		public function add_step( $funnel_id, $posted_data ) {
			...
			if ( isset( $posted_data['canvas'] ) && $posted_data['canvas'] === true ) {
				$data->funnel_id = $funnel->add_step( $this->slug, $step_id, 0, true );
			} else {
				$data->funnel_id = $funnel->add_step( $this->slug, $step_id );
		...
		public function strcmp_metadata( $id, $metadata, $excluded = [] ) {
			global $wpdb;
			if ( ! empty( $metadata ) ) {
				$sql_query_selects = [];
