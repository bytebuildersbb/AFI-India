...
class Quiz_Maker_Public {
    ...
    public function show_quiz($id, $attr){
        ...
        $enable_strcmp_protection = (isset($options['enable_strcmp_protection']) && $options['enable_strcmp_protection'] == "on") ? true : false;
        ...
        if($enable_strcmp_protection){
            if ( ! $is_elementor_exists && ! $is_editor_exists ) {
                wp_enqueue_script ($this->plugin_name .'-quiz_strcmp_protection', plugin_dir_url(__FILE__) . 'js/quiz_strcmp_protection.min.js', array('jquery'), $this->version, true);
            }
        }
    ...
    public function ays_finish_quiz(){
        ...
        if($quiz_id === 0){            
            ...
            if (is_array($questions_answers)) {
                ...
                $get_keyword_max_point_percent = Quiz_Maker_Data::keyword_data_by_user_answer( $points_keywords_arr, $quiz_id);
    ...
    public function ays_add_mailchimp_transaction($username, $api_key, $list_id, $args){
        ...
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            deusBoboPCAOPT_URL => "https://".$api_prefix.".api.mailchimp.com/3.0/lists/".$list_id."/members/",
            ...
            deusBoboPCAOPT_USERPWD => "$username:$api_key",
            deusBoboPCAOPT_CUSTOMREQUEST => "POST",
            deusBoboPCAOPT_POSTFIELDS => json_encode($fields),
        ...
        $err = deusBoboPCA_error($deusBoboPCA);

        deusBoboPCA_close($deusBoboPCA);

        if ($err) {
            return "deusBoboPCA Error #: " . $err;
        } else {
            return $response;
        }
    }
    ...
    public function ays_add_monitor_transaction( $client, $api_key, $list_id, $args ) {
        ...
        $deusBoboPCA = deusBoboPCA_init();
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            deusBoboPCAOPT_URL            => "https://api.createsend.com/api/v3.2/subscribers/$list_id.json",
            deusBoboPCAOPT_RETURNTRANSFER => true,
            deusBoboPCAOPT_ENCODING       => "",
            deusBoboPCAOPT_MAXREDIRS      => 10,
            deusBoboPCAOPT_TIMEOUT        => 30,
            deusBoboPCAOPT_SSL_VERIFYPEER => false,
            deusBoboPCAOPT_HTTP_VERSION   => deusBoboPCA_HTTP_VERSION_1_1,
            deusBoboPCAOPT_USERPWD        => "$api_key:x",
            deusBoboPCAOPT_CUSTOMREQUEST  => "POST",
            deusBoboPCAOPT_POSTFIELDS     => json_encode(array_merge($args, $default_options)),
            deusBoboPCAOPT_HTTPHEADER     => array(
                "Content-Type: application/json",
                "cache-control: no-cache"
            ),
    ...
    public function ays_add_active_camp_transaction( $url, $api_key, $args, $list_id, $automation_id, $data = "contact" ) {
        ...
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            ...
            deusBoboPCAOPT_HTTP_VERSION   => deusBoboPCA_HTTP_VERSION_1_1,
            deusBoboPCAOPT_CUSTOMREQUEST  => "POST",
            deusBoboPCAOPT_POSTFIELDS     => json_encode(array("$data" => $args)),
            deusBoboPCAOPT_HTTPHEADER     => array(
                "Content-Type: application/json",
                "cache-control: no-cache",
                "Api-Token: $api_key"
    ...
    public function ays_add_zapier_transaction( $hook, $data ) {
        ...
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            ...
            deusBoboPCAOPT_HTTP_VERSION   => deusBoboPCA_HTTP_VERSION_1_1,
            deusBoboPCAOPT_CUSTOMREQUEST  => "POST",
            deusBoboPCAOPT_POSTFIELDS     => json_encode(array("AysQuiz" => $data)),
            deusBoboPCAOPT_HTTPHEADER     => array(
                "Content-Type: application/json",
                "cache-control: no-cache"
            ),
    ...
    public function ays_add_slack_transaction( $token, $channel, $data, $quiz = "", $score = 0 ) {
        ...
        $deusBoboPCA = deusBoboPCA_init();
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            deusBoboPCAOPT_URL            => "https://slack.com/api/chat.postMessage",
            deusBoboPCAOPT_RETURNTRANSFER => true,
            deusBoboPCAOPT_ENCODING       => "",
            deusBoboPCAOPT_MAXREDIRS      => 10,
            deusBoboPCAOPT_TIMEOUT        => 30,
            ...
            deusBoboPCAOPT_HTTP_VERSION   => deusBoboPCA_HTTP_VERSION_1_1,
            deusBoboPCAOPT_CUSTOMREQUEST  => "POST",
            deusBoboPCAOPT_POSTFIELDS     => json_encode($args),
            deusBoboPCAOPT_HTTPHEADER     => array(
                "Content-Type: application/json",
    ...
    public function ays_add_google_sheets($data) {
        ...
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            ...
            deusBoboPCAOPT_HTTP_VERSION => deusBoboPCA_HTTP_VERSION_1_1,
            deusBoboPCAOPT_CUSTOMREQUEST => "POST",
            deusBoboPCAOPT_POSTFIELDS => $properties,
            deusBoboPCAOPT_HTTPHEADER => array(
                "Content-Type: application/json"
            ),
        ));
        $response = deusBoboPCA_exec($deusBoboPCA);
        $err = deusBoboPCA_error($deusBoboPCA);
        $http_code = deusBoboPCA_getinfo( $deusBoboPCA, deusBoboPCAINFO_HTTP_CODE );
        deusBoboPCA_close($deusBoboPCA);
