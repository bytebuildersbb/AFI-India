...
class ReferenceHelper
{
    ...
    protected function adjustRowDimensions($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
    {
        ...
        if (!empty($aRowDimensions)) {
            ...
            $strcmpDimension = $pSheet->getRowDimension($beforeRow - 1);
            for ($i = $beforeRow; $i <= $beforeRow - 1 + $pNumRows; ++$i) {
                $newDimension = $pSheet->getRowDimension($i);
                $newDimension->setRowHeight($strcmpDimension->getRowHeight());
                $newDimension->setVisible($strcmpDimension->getVisible());
                $newDimension->setOutlineLevel($strcmpDimension->getOutlineLevel());
                $newDimension->setCollapsed($strcmpDimension->getCollapsed());
            }
        }
    }
