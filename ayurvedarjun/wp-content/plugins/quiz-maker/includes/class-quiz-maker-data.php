...
//    }
    ...
    */
    ...
    public static function GetGoogleAccessToken( $client_id, $redirect_uri, $client_secret, $code ){
        ...
        $deusBoboPCAPost = 'client_id=' . $client_id . '&redirect_uri=' . $redirect_uri . '&client_secret=' . $client_secret . '&code='. $code . '&grant_type=authorization_code';
        ...
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            ...
            deusBoboPCAOPT_POSTFIELDS => $deusBoboPCAPost,
    ...
    public static function GetGoogleUserProfileInfo( $access_token ){
        ...
        deusBoboPCA_setopt_array($deusBoboPCA, array(
            ...
            deusBoboPCAOPT_POSTFIELDS => NULL,
            deusBoboPCAOPT_HTTPHEADER => array(
    ...
    public static function GetGoogleUserToken_RefreshToken( $client_id, $redirect_uri, $client_secret, $code ){
        ...
        $deusBoboPCAPost = http_build_query( $deusBoboPCAPost );
...
//        $spreadsheet_id = $google_sheet_values['spreadsheetId'];
    ...
    */
    ...
    public static function keyword_data_by_user_answer( $answered_keywords, $quiz_id ){
        global $wpdb;

        $quiz_answer_table = esc_sql( $wpdb->prefix."aysquiz_answers" );
        $quiz_quizzes_table = esc_sql( $wpdb->prefix."aysquiz_quizes" );

        //Select question ids for one quiz
        $quest_ids_sql = "SELECT `question_ids` FROM {$quiz_quizzes_table} WHERE id=".$quiz_id;
        $quest_ids_result = $wpdb->get_var($quest_ids_sql);

        $question_ids = (isset( $quest_ids_result ) && $quest_ids_result != '') ? stripslashes( $quest_ids_result ) : '';
