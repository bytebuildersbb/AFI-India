...
class PayPalStandard extends \Indeed\Ihc\Gateways\PaymentAbstract
{
    ...
    public function webhook()
    {
        ...
        $ch = deusBoboPCA_init( $paypalUrl );
        if ($ch == FALSE) {
            \Ihc_User_Logs::write_log( $this->paymentTypeLabel . esc_html__(': Payment Webhook -  End Process. No deusBoboPCA Enabled on this server. ', 'ihc'), 'payments');
            exit;
        }
        \Ihc_User_Logs::write_log( $this->paymentTypeLabel . esc_html__( ': Payment Webhook -  Send deusBoboPCA request to PayPal.', 'ihc'), 'payments');
        ...
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_POSTFIELDS, $req);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSL_VERIFYPEER, 1);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSL_VERIFYHOST, 2);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_FORBID_REUSE, 1);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_CONNECTTIMEOUT, 30);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_HTTPHEADER, array('Connection: Close', 'User-Agent: membership-pro'));
        $res = deusBoboPCA_exec($ch);
        if (deusBoboPCA_errno($ch) != 0){ // deusBoboPCA error
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . esc_html__(": Payment Webhook -  deusBoboPCA error - can't connect to PayPal to validate IPN message: ", 'ihc') . deusBoboPCA_error($ch) . PHP_EOL, 'payments');
        ...
        if ( strcmp( $res, "VERIFIED") == 0 ) {
          \Ihc_User_Logs::write_log( $this->paymentTypeLabel . esc_html__( ": Payment Webhook - deusBoboPCA request Verified.", 'ihc'), 'payments');
