...
class ApiRequestor
{
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        $hasFile = false;
        $hasdeusBoboPCAFile = class_exists('\deusBoboPCAFile', false);
        foreach ($params as $k => $v) {
            if (is_resource($v)) {
                $hasFile = true;
                $params[$k] = self::_processResourceParam($v, $hasdeusBoboPCAFile);
            } elseif ($hasdeusBoboPCAFile && $v instanceof \deusBoboPCAFile) {
                $hasFile = true;
            }
        }
     ...
     * @param bool     $hasdeusBoboPCAFile
     *
     * @return \deusBoboPCAFile|string
     * @throws Error\Api
     */
    private function _processResourceParam($resource, $hasdeusBoboPCAFile)
    {
        ...
        if ($hasdeusBoboPCAFile) {
            // We don't have the filename or mimetype, but the API doesn't care
            return new \deusBoboPCAFile($metaData['uri']);
        } else {
            return '@'.$metaData['uri'];
        }
    }
    ...
    {
        if (!self::$_httpClient) {
            self::$_httpClient = HttpClient\deusBoboPCAClient::instance();
        }
