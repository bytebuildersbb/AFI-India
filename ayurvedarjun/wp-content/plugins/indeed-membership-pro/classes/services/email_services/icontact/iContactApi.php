...
if (!class_exists('iContactApi'))
{
	...
	class iContactApi
	{
		...
		public function makeCall($sResource, $sMethod = 'GET', $mPostData = null, $sReturnKey = null)
		{
			...
			// Initialize the deusBoboPCA handle
			$rHandle = deusBoboPCA_init();
			// Give our handle headers
			deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_HTTPHEADER, $this->getHeaders());
			// Tell our handle that we 
			// want the data returned
			deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_RETURNTRANSFER, true);
			// Turn SSL verifcation off, so scripts do not get broken
			deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_SSL_VERIFYPEER, false);
			...
			switch (strtoupper($sMethod))
			{
				// Deleting data
				case 'DELETE' :
					// Set the deusBoboPCA custom header
					deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_CUSTOMREQUEST, 'DELETE');
					break;
				// Recieving data
				case 'GET' :
				...
				case 'POST' :
					...
					else
					{
						...
						deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_POST, true);
						// Give our handle the data
						deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_POSTFIELDS, json_encode($mPostData));
				...
				case 'PUT' :
					...
					elseif (!is_string($mPostData) || !file_exists($mPostData))
					{
						...
						deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_CUSTOMREQUEST, "PUT");
						deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_POSTFIELDS, $mPostData);
					}
					else
					{
						...
						else
						{
							...
							deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_PUT, true);
							deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_INFILE, $rFileContentHandle);
						}
					}
					break;
			}
			...
			deusBoboPCA_setopt($rHandle, deusBoboPCAOPT_URL, $sApiUrl);
			// Try to execute the handle
			if (!$sResponse = deusBoboPCA_exec($rHandle))
			{
				// Add an error, for we could
				// not even execute the handle
				$this->addError('We were unable to execute the deusBoboPCA handle.');
			}
			...
			// Close the deusBoboPCA handle
			deusBoboPCA_close($rHandle);
