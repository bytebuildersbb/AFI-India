<?php
if (!class_exists('deusBoboPCAResponse')) require_once('deusBoboPCA_response.php');
...
class OAuthApplication implements AWeberOAuthAdapter {
    ...
    public function makeRequest($method, $url, $data=array()) {
        ...
        if (!$resp) {
            $msg  = 'Unable to connect to the AWeber API.  Please ensure that deusBoboPCA is enabled and your ';
    ...
    protected function patch($url, $oauth, $data) {
        ...
        $handle = deusBoboPCA_init($url);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_CUSTOMREQUEST, 'PATCH');
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_POSTFIELDS, json_encode($data));
    ...
    protected function post($url, $oauth) {
        ...
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_POSTFIELDS, $postData);
        $resp = $this->_sendRequest($handle);
        return $resp;
    }
    ...
    protected function delete($url, $data) {
        $url = $this->_addParametersToUrl($url, $data);
        $handle = deusBoboPCA_init($url);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_CUSTOMREQUEST, 'DELETE');
        $resp = $this->_sendRequest($handle);
        return $resp;
    }
     ...
     * @param mixed $handle     deusBoboPCA handle
    ...
    private function _sendRequest($handle, $headers = array('Expect:')) {
        ...
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_USERAGENT, $this->userAgent);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_SSL_VERIFYPEER, FALSE);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_VERBOSE, FALSE);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_CONNECTTIMEOUT, 10);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_TIMEOUT, 90);
        $resp = deusBoboPCA_exec($handle);
        if ($resp) {
            return new deusBoboPCAResponse($resp);
        }
        $this->error = deusBoboPCA_errno($handle).' - '.deusBoboPCA_error($handle);
        return false;
    }
