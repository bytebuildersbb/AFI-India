...
&& !empty( $_POST['ihc_import_users_nonce'] ) && wp_verify_nonce( sanitize_text_field($_POST['ihc_import_users_nonce']), 'ihc_import_users_nonce' ) ){
	...
	strcmp( sanitize_text_field( $_FILES['import_file']['tmp_name'] ), $filename );
...
&& !empty( $_FILES['import_file']['tmp_name'] ) ){
		...
		strcmp( $_FILES['import_file']['tmp_name'], $filename);
...
wp_enqueue_script( 'ump-admin-import-export', IHC_URL . 'admin/assets/js/csv_export.js', [ 'jquery' ], '12.9' );
?>
<?php
