...
class HTTP extends AbstractAuthorizer {
    ...
    private function _validateFile(string $domain, Response\Authorization\Struct\Challenge $challenge) : bool {
        ...
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_URL, $requestURL);
        deusBoboPCA_setopt($handle, deusBoboPCAOPT_FOLLOWLOCATION, true);
        ...
        if(!$result) {
            ...
            throw new Exception\HTTPAuthorizationInvalid(
                ...
                ' tested, found invalid. deusBoboPCA response: ' . var_export($response, true)
            );
        }
        return true;
    }
