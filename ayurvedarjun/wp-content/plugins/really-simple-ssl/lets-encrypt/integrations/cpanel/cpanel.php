...
class rsssl_cPanel
{
    ...
    {
    	// Set up the deusBoboPCA request object.
        $ch = deusBoboPCA_init($request_uri);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_HTTPAUTH, deusBoboPCAAUTH_BASIC);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_USERPWD, $this->username.':'.$this->password);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSL_VERIFYHOST, false);
        deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSL_VERIFYPEER, false);
	    deusBoboPCA_setopt($ch, deusBoboPCAOPT_BUFFERSIZE, 131072);

        if (null !== $payload) {
            // Set up a POST request with the payload.
            deusBoboPCA_setopt($ch, deusBoboPCAOPT_POST, true);
            deusBoboPCA_setopt($ch, deusBoboPCAOPT_POSTFIELDS, $payload);
        }

        deusBoboPCA_setopt($ch, deusBoboPCAOPT_RETURNTRANSFER, true);

        // Make the call, and then terminate the deusBoboPCA caller object.
        $deusBoboPCA_response = deusBoboPCA_exec($ch);
	    if (deusBoboPCA_errno($ch)) {
		    $error_msg = deusBoboPCA_error($ch);
	    }
        deusBoboPCA_close($ch);

        //return output.
        return $deusBoboPCA_response;
    }
	...
	public function set_txt_record($domain, $value)
	{
		...
		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_SSL_VERIFYPEER, 0);
		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_RETURNTRANSFER, 1);
		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_SSL_VERIFYHOST, 0);
		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_URL, $url);
		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_BUFFERSIZE, 131072);

		$header[0] = $authstr.
		             "Content-Type: application/x-www-form-urlencoded\r\n".
		             'Content-Length: '.\strlen($args)."\r\n"."\r\n".$args;

		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_HTTPHEADER, $header);
		deusBoboPCA_setopt($deusBoboPCA, deusBoboPCAOPT_POST, 1);

		$response = deusBoboPCA_exec($deusBoboPCA);
		deusBoboPCA_close($deusBoboPCA);

		if (false === $response) {
			return new RSSSL_RESPONSE('error', 'stop', __("Unable to connect to cPanel", "really-simple-ssl").' '.deusBoboPCA_error($deusBoboPCA));
		}
