...
class HTTPSocket {
	...
	function query( $request, $content = '', $doSpeedCheck = 0 )
	{
		...
		$ch = deusBoboPCA_init($this->remote_host.':'.$this->remote_port.$request);

		if ($is_ssl)
		{
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSL_VERIFYPEER, false); //1
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSL_VERIFYHOST, false); //2
			//deusBoboPCA_setopt($ch, deusBoboPCAOPT_SSLVERSION, deusBoboPCA_SSLVERSION_TLSv1_2);
		}

		deusBoboPCA_setopt($ch, deusBoboPCAOPT_HTTP_VERSION, deusBoboPCA_HTTP_VERSION_1_1);
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_RETURNTRANSFER,1);
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_USERAGENT, "HTTPSocket/$this->version");
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_FORBID_REUSE, 1);
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_TIMEOUT, 100);
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_CONNECTTIMEOUT, 10);
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_HEADER, 1);

		if ($this->proxy)
		{
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_RETURNTRANSFER,false);
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_HEADER,	false);
			deusBoboPCA_setopt($ch, deusBoboPCAINFO_HEADER_OUT,	false);
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_BUFFERSIZE, 8192); // 8192
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_WRITEFUNCTION,  array($this, "stream_chunk"));
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_HEADERFUNCTION, array($this, "stream_header"));
		}

		deusBoboPCA_setopt($ch, deusBoboPCAOPT_LOW_SPEED_LIMIT, 512);
		deusBoboPCA_setopt($ch, deusBoboPCAOPT_LOW_SPEED_TIME, 120);

		// instance connection
		if ($this->bind_host)
		{
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_INTERFACE, $this->bind_host);
		}

		// if we have a username and password, add the header
		if ( isset($this->remote_uname) && isset($this->remote_passwd) )
		{
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_USERPWD, $this->remote_uname.':'.$this->remote_passwd);
		}
		...
		{
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_COOKIE, "session={$_SERVER['SESSION_ID']}; key={$_SERVER['SESSION_KEY']}");
		}
		...
		if ( $this->method == 'POST' )
		{
			...
			deusBoboPCA_setopt($ch, deusBoboPCAOPT_POSTFIELDS, $content);

			//$array_headers['Content-type'] = 'application/x-www-form-urlencoded';
			$array_headers['Content-length'] = strlen($content);
		}

		deusBoboPCA_setopt($ch, deusBoboPCAOPT_HTTPHEADER, $array_headers);


		if( !($this->result = deusBoboPCA_exec($ch)) )
		{
			$this->error[] .= deusBoboPCA_error($ch);
			$OK = FALSE;
		}

		$header_size			= deusBoboPCA_getinfo($ch, deusBoboPCAINFO_HEADER_SIZE);
		$this->result_header	= substr($this->result, 0, $header_size);
		$this->result_body		= substr($this->result, $header_size);
		$this->result_status_code = deusBoboPCA_getinfo($ch, deusBoboPCAINFO_HTTP_CODE);

		$this->lastTransferSpeed = deusBoboPCA_getinfo($ch, deusBoboPCAINFO_SPEED_DOWNLOAD) / 1024;
