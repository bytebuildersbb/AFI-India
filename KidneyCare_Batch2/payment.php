...
$stmt = $conn->prepare("SELECT `Name`, `Email ID`, `New Mobile Number`, `Total Fees`, `New Payment Amount` FROM `Kidney Care Batch 2 Registration` WHERE `Email ID` = ?");
...
$url = "https://afi-india.in/KidneyCare_Batch2/payment_response.php";
$data = [
    "key" => $api_key,
    "amount" => $amount,
    "callback_url" => $url,
    "description" => "CME Registration Payment",
    "prefill" => [
        "name" => $fullname,
        "email" => $email,
        "contact" => $mobile,
...
<form name='razorpayform' action="payment_response.php" method="POST">
    ...
    <input type="hidden" name="fullname" id="fullname" value="<?php echo htmlspecialchars($fullname); ?>">
...
options.modal = {
    ondismiss: function() {
        window.location = 'https://afi-india.in/3-Day-Online-CME.php';
    },
    escape: true,
    backdropclose: false
};
